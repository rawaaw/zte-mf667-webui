define(['underscore', 'config/config', 'smsData'],function (_, config, smsData) {
    ...
    var simulate = {
		simulateRequest:function (params, successCallback, errorCallback, async, isPost) {
            if (!!isPost) {
                if (params.goformId == "USSD_PROCESS"){
		  processUSSD(params);
                }
...
//alert('2params.cmd:'+params.cmd);
    	...
    	*/
    	...
    	ipv6_pdp_type: 'IP',
        dns_pri_addr_v4:"192.168.0.25" ,
        dns_sec_addr_v4:"192.168.0.100" ,
        dns_pri_addr_v6:"192.168.0.160" ,
        dns_sec_addr_v6:"192.168.0.253" ,
        model_name:"mf823",
	...
	setInterval(function(){
        ...
        if(!simulate.testEnv){
	        if(getRandomInt(60000) == 0){ // 提高接收频率可将此值改小，如：2
    ...
    function setUpgradeSelectOption(params) {
        ...
        if (params.select_op == "check") {
            simulate.current_upgrade_state = "checking";
    ...
    function processUSSD(params) {
alert('processUSSD:->'+params.USSD_operator +'<- ->'+params.USSD_send_number);
	if(params.USSD_operator == "ussd_send"){
	}else if(params.USSD_operator == "reply"){

        }else {

        }
	simulate.ussd_result = 1;
